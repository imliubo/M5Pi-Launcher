...
static lv_obj_t *imliubo_menu_btn;

static lv_obj_t *dock_menu_list[] = {
    &linux_menu_btn, &pi_menu_btn, &imliubo_menu_btn
};
...
LV_IMG_DECLARE(IAMLIUBO);
static const lv_img_dsc_t *dock_icon_list[] = {
    &linux_menu, &pi_menu, &IAMLIUBO
};
...
void launcher_widgets(void)
{
    ...
    lv_obj_set_size(desktop_tbv, 320, 240);
...
static void dock_menu_event_handler(lv_event_t *e)
{
    ...
    if (obj == dock_menu_list[0])
    {
        ...
        lv_obj_set_size(app_win, 320, 240);
        lv_canvas_set_buffer(app_win, cbuf, 320, 240, LV_IMG_CF_TRUE_COLOR);
    ...
    else if (obj == dock_menu_list[1])
    {
        ...
        lv_qrcode_update(qr, "https://github.com/imliubo/M5Pi", strlen("https://github.com/imliubo/M5Pi"));
        lv_obj_align(qr, LV_ALIGN_CENTER, 0, 0);

        lv_obj_t * label = lv_label_create(app_win);
        lv_label_set_text(label, "M5Pi Project Github Repo");
    ...
    else if (obj == dock_menu_list[2])
    {
        ...
        lv_canvas_fill_bg(app_win, lv_color_make(0xFF, 0xFF, 0xFF), 0);
        lv_obj_t * qr = lv_qrcode_create(app_win, 150, lv_color_make(255, 255, 255), lv_color_make(0, 0, 0));
        lv_qrcode_update(qr, "https://space.bilibili.com/31532108", strlen("https://space.bilibili.com/31532108"));
        lv_obj_align(qr, LV_ALIGN_CENTER, 0, 0);
